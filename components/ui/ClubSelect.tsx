...
const ClubSelect: React.FC<ClubSelectProps> = ({ onClubChange, allClubsData }) => {
  const [selectedClub, setSelectedClub] = React.useState<ClubFormValues | null>(null);
  return (
          ...
          <div className="relative mt-2 mb-4 md:px-6 lg:px-8">
            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
              <span className="flex items-center">
                <img src={selectedClub?.logo ? selectedClub.logo : 'https://res.cloudinary.com/dajtykxvp/image/upload/v1701640413/logos/bishl_logo.png'} alt="" className="h-5 w-5 flex-shrink-0 rounded-full" />
                    ...
                    {({ selected, active }) => (
                      <>
                        <div className="flex items-center">
                          <img src={club?.logo ? club.logo : 'https://res.cloudinary.com/dajtykxvp/image/upload/v1701640413/logos/bishl_logo.png'} alt="" className="h-5 w-5 flex-shrink-0 rounded-full" />
                          <span
