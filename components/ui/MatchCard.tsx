      ...
      <div className="flex flex-col sm:w-1/3">
        ...
        <div className="flex flex-row sm:flex-col justify-between sm:justify-end mt-3 sm:mt-0 sm:pr-4 sm:gap-y-2 sm:h-full">
          ...
          <div className="flex items-center">
            ...
            <p className="text-xs uppercase font-light text-gray-700 mb-0"><time dateTime={(new Date(startDate)).toISOString()}>{(new Date(startDate)).toLocaleString('de-DE', { 
      ...
      <div className="flex flex-col gap-y-2 sm:gap-x-2 justify-between mt-3 sm:mt-0 w-full sm:w-1/2">
        ...
        <div className="flex flex-row items-center w-full">
          ...
          <div className="flex-auto ml-6">
            <p className={`${isTablet ? 'text-base' : 'text-lg'} font-medium leading-6 ${home.stats.goalsFor > away.stats.goalsFor ? 'text-gray-800' : 'text-gray-500'}`}>{home.fullName}</p>
          </div>
          <div className="flex-auto">
            <p className={`${isTablet ? 'text-base' : 'text-lg'} font-medium leading-6 ${home.stats.goalsFor > away.stats.goalsFor ? 'text-gray-800' : 'text-gray-500'} text-right mx-2`}>{home.stats.goalsFor}</p>
          </div>
        </div>
        {/* away */}
        <div className="flex flex-row items-center w-full">
            ...
            <div className="flex-auto ml-6">
              <p className={`${isTablet ? 'text-base' : 'text-lg'} font-medium leading-6 ${away.stats.goalsFor > home.stats.goalsFor ? 'text-gray-800' : 'text-gray-500'}`}>{away.fullName}</p>
            </div>
            <div className="flex-auto">
              <p className={`${isTablet ? 'text-base' : 'text-lg'} font-medium leading-6 ${away.stats.goalsFor > home.stats.goalsFor ? 'text-gray-800' : 'text-gray-500'} text-right mx-2`}>{away.stats.goalsFor}</p>
            </div>
          </div>
      </div>
